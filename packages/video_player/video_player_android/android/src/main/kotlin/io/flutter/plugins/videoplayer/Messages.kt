...
) : PlatformVideoEvent()
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): InitializationEvent {
      ...
      return InitializationEvent(duration, width, height, rotationCorrection, isLive)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
interface VideoPlayerInstanceApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: VideoPlayerInstanceApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.isLive$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              listOf(api.isLive())
